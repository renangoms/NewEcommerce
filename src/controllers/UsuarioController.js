...
class UsuarioController {
  ...
  store(req, res, next) {
    const {
      nome, email, password, confirmPassword, loja,
    } = req.body;

    if (!nome || !email || !password || !confirmPassword) return res.status(422).json({ errors: 'Preencha todos os campos de cadastro.' });
    if (password !== confirmPassword) return res.status(422).json({ errors: 'Senhas diferentes' });
  ...
  update(req, res, next) {
    ...
    Usuario.findById(req.payload.id).then((usuario) => {
      ...
      return usuario.save().then(() => res.json({ usuario: usuario.enviarAuthJSON() })).catch(next);
    }).catch(next);
  }
  ...
  remove(req, res, next) {
    Usuario.findById(req.payload.id).then((usuario) => {
      if (!usuario) return res.status(401).json({ errors: 'Usuário não registrado' });
      return usuario.remove().then(() => res.json({ deletado: true })).catch(next);
    }).catch(next);
  }
  ...
  createRecovery(req, res, next) {
    ...
    Usuario.findOne({ email }).then((usuario) => {
      ...
      return usuario.save().then(() => {
        enviarEmailRecovery({ usuario, recovery: recoveryData }, (error = null, success = null) => res.render('recovery', { error, success }));
      }).catch(next);
    }).catch(next);
  }
  ...
  completeRecovery(req, res, next) {
    ...
    if (!token || !password) return res.render('recovery/store', { error: 'Preencha novamente com sua nova senha', success: null, token });
    Usuario.findOne({ 'recovery.token': token }).then((usuario) => {
      ...
      usuario.setSenha(password);
      return usuario.save().then(() => res.render('recovery/store', {
        error: null,

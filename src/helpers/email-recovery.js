const transportar = require('nodemailer').createTransport(require('../config/email'));
...
module.exports = ({ usuario, recovery }, cb) => {
  ...
  if (process.env.NODE_ENV === 'production') {
    transportar.sendEmail(opcoesEmail, (error) => {
      if (error) {
        console.log(error);
        return cb('Aconteceu um erro no envio do email, tente novamente.');
      }
      return cb(null, 'Aconteceu um erro no envio do email, tente novamente.');
    });
  } else {
    console.log(opcoesEmail);
    return cb(null, 'Link por redefinicao de senha foi enviado com sucesso para seu email.');
  }
};
